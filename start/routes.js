...
Route.group(() => {
  ...
  //Rutas de la agenda
  Route.get('proyectos/:id/tareas/:id/agenda', 'AgendaController.index').middleware('auth');
  Route.post('proyectos/:id/tareas/:id/agenda', 'AgendaController.create').middleware('auth');
  Route.patch('agenda/:id', 'AgendaController.update').middleware('auth');
  Route.delete('agenda/:id', 'AgendaController.destroy').middleware('auth');
}).prefix('api/v1/');
